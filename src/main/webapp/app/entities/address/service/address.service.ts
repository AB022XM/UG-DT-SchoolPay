...
type RestOf<T extends IAddress | NewAddress> = Omit<T, 'createdAt' | 'updatedAt' | 'isDeleted'> & {
...
export class AddressService {
  ...
  protected convertDateFromClient<T extends IAddress | NewAddress | PartialUpdateAddress>(address: T): RestOf<T> {
    return {
      ...
      isDeleted: address.isDeleted?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAddress: RestAddress): IAddress {
    return {
      ...
      isDeleted: restAddress.isDeleted ? dayjs(restAddress.isDeleted) : undefined,
    };
  }
